      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 space-y-6 max-w-7xl">
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold text-slate-900">Student Booking Requests Queue</h1>
              <p className="text-xs text-slate-500 mt-1">Review incoming 1-on-1 session requests, accept or reject bookings, and open chat communication with students.</p>
            </div>
              ...
              bookings.map((b) => (
                <div key={b.id} className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                          {b.student_name || 'Student Request'}
                    ...
                    <p className="text-xs text-slate-600 font-medium">📅 Date: {b.scheduled_date} • ⏰ Time: {b.start_time} - {b.end_time}</p>

                    {b.student_requirement && (
                      <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 text-xs space-y-1">
                        <p className="text-[10px] uppercase font-bold text-blue-700 flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold flex items-center gap-1.5 border border-slate-200"
                    >
                    ...
                    {b.status === 'requested' || b.status === 'pending' ? (
                      <>
                        <button
                          ...
                          className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center gap-1 shadow-2xs"
                        >
                          <Check className="w-4 h-4" /> Accept Session
                        </button>
                        <button
                          onClick={() => handleAction(b.id, 'rejected')}
                          className="px-4 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold border border-rose-200"
                        >
                          Decline Request
                        </button>
                      </>
                      ...
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-full uppercase ${
                        b.status === 'accepted' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                      }`}>
                        {b.status}
                      </span>
                    )}
