      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 space-y-6 max-w-7xl">
          <div className="glass-card rounded-3xl p-6 border border-slate-200 bg-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
            <div>
              <h1 className="text-xl font-extrabold text-slate-900">Verified Scholarship Source Portal</h1>
            ...
            <button className="px-4 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold flex items-center gap-1.5 shadow-sm">
          ...
          <div className="space-y-4">
            {scholarships.map((s) => (
              <div key={s.id} className="glass-card rounded-3xl p-6 border border-slate-200 bg-white flex items-center justify-between shadow-sm">
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="text-base font-bold text-slate-900">{s.name}</h3>
                    <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                      <ShieldCheck className="w-3 h-3 text-emerald-600" /> {s.status}
                    </span>
                  </div>
                  <p className="text-xs text-indigo-700 font-bold mt-0.5">Provider: {s.provider}</p>
                  <p className="text-xs text-slate-500 font-medium mt-1">Application Deadline: {s.deadline} • Verified: {s.lastVerified}</p>
                </div>
